...
import { createDriverDemo, DEMO_ID, SCENES, driverActions, latestOffer, hosRings, dutySegments, recordAction } from './model.js'
...
export function DriverSignIn() {
  ...
  function submit(e) {
    ...
    const r = signInDriver(d.get('truck'), d.get('pin'))
    if (!r.ok) setError(r.error); else location.hash = '/driver/today'
  }
  return <div className="dp-shell"><main className="dp-login"><a className="dp-wordmark" href="#/board">Gladiolus<span>DRIVER</span></a><div className="dp-login-art"><Icon name="truck" /><span>Every mile, a little clearer.</span></div><section className="dp-card"><div className="dp-heading"><span className="dp-eyebrow">WELCOME BACK</span><h1>Ready for the road?</h1><p>Sign in to your assigned truck.</p></div><form className="dp-form" onSubmit={submit}><label>Truck number<input name="truck" required defaultValue={SEED_DRIVERS[0].truckId} autoComplete="username" /></label><label>Driver PIN<input name="pin" type="password" inputMode="numeric" pattern="[0-9]{4}" maxLength={4} defaultValue={SEED_DRIVERS[0].pin} required autoComplete="current-password" /></label>{error && <p role="alert" className="dp-error">{error}</p>}<Button type="submit">Start my shift</Button></form><a className="dp-text-link" href="#/driver-demo/today">Explore the interactive demo →</a><p className="dp-footnote">Prototype sign-in · Demo credentials are filled in.</p></section></main></div>
}
...
export default function DriverPortal({ demoController, incidents = INITIAL_INCIDENTS }) {
  const demo = !!demoController, { user, signOut } = useAuth(), store = useStore(), sim = useSim(), world = useWorld()
  ...
  const [navigating, setNavigating] = useState(false), [sheet, setSheet] = useState('medium')
  ...
  function submitRequest(e, action) {
    ...
    if (act(action, { message, reason: f.get('reason') || null, siteId: dock?.id || null, loadId: truck.loadId, drivingMs: truck.drivingMs })) { go('requests'); notify('Request recorded in the dispatch event log.') }
  }
  ...
  function record(type, payload) { store.append(type, world.clock, { truckId, ...payload }); store.commit() }
  function submitInspection(e) {
    ...
    record(EVENT.INSPECTION, { phase, defects, major, odometerKm: truck.odometerKm, note: String(f.get('note') || '').trim() })
    go('inspections')
  ...
  function submitBreakdown(e) {
    ...
    record(EVENT.BREAKDOWN, { category, blockingLane: f.get('blockingLane') === 'on', coord: truck.coord, chainage: truck.chainage,
  ...
  switch (screen) {
    case 'today': content = <>
      ...
      {breakdown ? <div className="dp-breakdown-card"><span className="dp-eyebrow">REPORTED {time(breakdown.at)}</span><h2>{CATEGORY_BY_ID[breakdown.category]?.label || 'Breakdown'}</h2><p>Dispatch has your position, your load and your remaining hours. Stay with the truck unless it is unsafe.</p><Button onClick={() => go('breakdown')}>View your report <Icon name="arrow" /></Button></div> : <>
        ...
        {dock ? <div className="dp-dock"><ShipmentVisit truck={{...truck, clock: world.clock}} stopId={truck.insideSiteId} /><div className="dp-actions"><Button onClick={() => go('delay')}>Report delay</Button><Button secondary onClick={() => go('help')}>Dispatch</Button></div></div>
    ...
    case 'offer': content = offer && world.clock < offer.expiresAt ? <><span className="dp-pill">NEW OFFER · FTL</span><div className="dp-heading"><h2>{SITE_BY_ID[offer.originId]?.name}<span className="dp-route-arrow">↓</span>{SITE_BY_ID[offer.destinationId]?.name}</h2><p>{offer.offerId}</p></div><div className="dp-stats"><span><b>{offer.distanceKm} km</b>Trip distance</span><span><b>{time(offer.dueAt)}</b>Delivery due</span><span><b>{offer.weightKg/1000} t</b>Load weight</span></div><p className="dp-info">{left >= offer.distanceKm / 80 * H ? 'Enough driving and duty time for the estimated trip.' : 'Not enough hours for this trip. Contact dispatch.'}</p><p className="dp-center">Offer expires in {fmtClock(offer.expiresAt-world.clock)}</p><div className="dp-actions"><Button onClick={() => { if(act('load.accepted', {offerId:offer.offerId})) {go('load');notify('Load accepted. Your assignment has been updated.')} }}>Accept load</Button><Button secondary onClick={() => { if(act('load.rejected', {offerId:offer.offerId})) {go('today');notify('Load declined.')} }}>Reject</Button></div></> : <Empty title="No pending offers" action={<Button onClick={() => go('today')}>Back to Today</Button>}>Accepted, declined, and expired offers leave this queue.</Empty>; break
    ...
    case 'requests': content=<><div className="dp-heading"><h2>Your requests</h2><p>Recorded in this session</p></div>{requestList.length ? requestList.map(e=><article className="dp-request" key={e.seq}><div><strong>{e.action==='delay.reported'?'Delay report':e.action==='correction.requested'?'Log correction':'Dispatch request'}</strong><span className="dp-pill">Recorded</span></div><p>{e.message}</p><small>{time(e.at)}{e.reason?` · ${e.reason}`:''}</small></article>) : <Empty title="You’re all caught up">No requests have been recorded yet.</Empty>}<Button secondary onClick={()=>go('help')}>New request</Button></>;break
  ...
  return <div className={`dp-shell ${demo?'dp-is-demo':''}`}>
    ...
    <main className={`dp-app has-map sheet-${sheet} map-${mapMode}`}>
      ...
      <header className="dp-header"><a href={demo?'#/driver-demo/today':'#/driver/today'} className="dp-wordmark">Gladiolus<span>DRIVER</span></a><span className="dp-clock"><i /> {time(world.clock)}</span></header>
